...
do
        if [ -f "$inputfile" ] ; then
                ...
                if [ "$charset" != "utf-8" ]; then
						# ABES / MSR : if CHARSET=unknown-8bit change it to CHARSET=iso-8859-1
                        ...
                        if in_array $extension "${filestoconvert[@]}" ; then
                                ...
                                mv "$tmp" "$inputfile"
								# ABES / MSR : Modify or add CHARSET=UTF-8
								if ! grep -q "(<META\s|<meta\s)" $inputfile ; then
								  sed -i 's#</TITLE>#</TITLE>\n<META HTTP-EQUIV="Content-Type" CONTENT="text/html; CHARSET=UTF-8">#g' $inputfile &>2
								  sed -i 's#</title>#</title>\n<META HTTP-EQUIV="Content-Type" CONTENT="text/html; CHARSET=UTF-8">#g' $inputfile &>2
								else
								  sed -i 's/(CHARSET|charset)=iso-8859-1/CHARSET=UTF-8/g' $inputfile &>2
								fi
